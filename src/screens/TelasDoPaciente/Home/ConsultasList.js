...
const ConsultasList = ({ consultas, onEdit }) => {
      ...
      >
        {consultas.map((item) => (
          <View key={item.id} style={styles.card}>
            <Text style={styles.name}>Especialidade: {item.specialty}</Text>
            <Text style={styles.details}>Data: {item.date} | Hora: {item.time}</Text>
            <Text style={styles.details}>Médico: {item.doctor}</Text>
...
const styles = StyleSheet.create({
  ...
  editButton: {
    backgroundColor: '#007aff', // Cor para o botão de editar
    padding: 10,
    borderRadius: 5,
    marginTop: 10,
